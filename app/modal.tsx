...
import { Alert, FlatList, Image, Linking, Pressable, Modal as RNModal, StyleSheet, Text, TextInput, TouchableOpacity, View } from "react-native";
...
export default function Modal() {
    ...
    const handlePost = () => {console.log('handlePost')};
    ...
    const removeThread = (id: string) => {
        setThreads((prevThreads) =>
            prevThreads.filter((threads) => threads.id !== id )
        );
    };
    ...
    const pickImage = async (id: string) => {
      ...
      if( status !== "granted" ){
        Alert.alert("Photos permisstion not granted",
      ...
      if( !result?.canceled ){
        ...
        setThreads((prevThreads) => 
          prevThreads.map((thread) => 
            thread.id === id ? { ...thread, imageUris: result.assets?.map((asset) => asset.uri) ?? [] } : thread
          )
        )
      }
    };
    ...
    const getMyLocation = async (id: string) => {
      ...
      try {
        ...
        Alert.alert("좌표값 오류", "좌표를 찾는데 오류가 발생했습니다.");
        return;
      }
            ...
            <View style={styles.contentContainer}>
                <View style={styles.userInfoContainer}>
                    ...
                    {index > 0 && (
                        ...
                        >
                            <Ionicons name="close-outline" size={20} colo="#8e8e93" />
                        </TouchableOpacity>
                    )}
                </View>
                <TextInput 
